...
from .diffusion_model.diffuse_plume import (
    Field, 
    Source, 
    DiffusePlumeLidar
)
...
class Gas:
    def __init__(self, Q: float, offset: float, cross_section):
        ...
        self.concentration: np.ndarray = np.ndarray([])
...
class Enviroment:
    ...
    def transmittance(self, lidar, wl):
        ...
        absorptance_aer = alphas_aer(
            wl[np.newaxis, :], lidar.z_grid[:, np.newaxis], self.aer_absorp_feat
        )
        absorptance_gas = {
            name: obj.concentration[:, np.newaxis]
            * obj.cross_section(wl[np.newaxis, :])
        ...
        intgr = np.cumsum(
            ...
            * np.diff(lidar.distance)[:, np.newaxis]
            * 0.5,
            axis=0,
        )
...
class Measurement:
    def __init__(
        ...
        for name, obj in self.env.gases.items():
            obj.concentration = (
                (obj.offset + obj.Q * self.C) * 1e-6 * N(self.lidar.z_grid)
            )
        ...
        *,
        ...
        if gas_dict is not None:
            for k, v in gas_dict.items():
                self.env.gases = {k: v for k, v in gas_dict.items()}
    ...
    def show_gases(self):
        ...
        ax2.plot(
            r,
            self.env.diffuse.Concentration(
                r, z=self.lidar.alt_offset, time_correction=10 * 60
            ),
            c="darkgrey",
            clip_on=False,
        )
