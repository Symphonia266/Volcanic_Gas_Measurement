...
def plotter(x, y, C, func, x_obs):
    ...
    ax2.view_init(elev=20, azim=-155)
...
x_src, y_src, q_src = gen_fauntainsource(5, (50, -5), 30)
He_src = 2

model.entry_source(q_src, x_src, y_src, He_src)
C = model.Concentration(x_grid, y=y_grid, z=z_grid).T
print(C.shape)
plotter(x_grid, y_grid, C, model.Concentration, x_obs)
model.clear_source()

x_src, y_src, q_src = gen_fauntainsource(5, (50, -25), 5)
model.entry_source(q_src, x_src, y_src, He_src)
C = model.Concentration(x_grid, y=y_grid, z=z_grid).T
plotter(x_grid, y_grid, C, model.Concentration, x_obs)
model.clear_source()

model = DiffusePlumeLidar("pasquill", 10, 0, stab_class="D")
x_src, y_src, q_src = gen_fauntainsource(5, (30, 0), 30)
model.entry_source(q_src, x_src, y_src, He_src)
C = model.Concentration(x_grid, y=y_grid, z=z_grid).T
plotter(x_grid, y_grid, C, model.Concentration, x_obs)
model.clear_source()
