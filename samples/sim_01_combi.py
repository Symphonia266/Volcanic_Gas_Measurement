...
for (s1, s2), res_obj in results.items():
    ...
    ax2.errorbar(
        ...
        ecolor=my_cm(i), color=my_cm(i),
        capsize=4, fmt='o', markersize=3, zorder=num-i,
        label=f"{s1:>13}-{s2:>13}", 
    )
    ax7.errorbar(
        x=res_obj.coord.distance, 
    ...
    if (s1,s2) == ("O2 Stokes", "O2 a-Stokes") or (s1,s2) == ("N2 Stokes", "O2 Stokes"):
        ax2_ins.errorbar(
            x=res_obj.coord.distance, 
            y=utils.number_density_to_ppm(
                res_obj.res[:, idx_trgt]-cfs[(s1, s2)][:, idx_trgt], 
    ...
    contam_error = utils.number_density_to_ppm(
        np.abs(
            res_obj.res[-1,:]
            -cfs[(s1, s2)][-1, :]
            -res_obj.n_true["SO2"][-1]
        ), 
        res_obj.coord.z[-1]
    )
    stat_error = utils.number_density_to_ppm(
        np.abs(res_obj.stat_err[-1, :]), 
        res_obj.coord.z[-1]
    )
    
    ax3.scatter(
        wl_laser[~np.isnan(contam_error)], 
        contam_error[~np.isnan(contam_error)],         
        color=my_cm(i), label=f"{s1}-{s2}", s=6, zorder=num-i
    )
    ax4.scatter(
        wl_laser[~np.isnan(stat_error)], 
        stat_error[~np.isnan(stat_error)], 
        color=my_cm(i), label=f"{s1}-{s2}", s=6, zorder=num-i
    )
    ...
    if (s1,s2) == ("O2 Stokes", "O2 a-Stokes") or (s1,s2) == ("N2 Stokes", "O2 Stokes"):
        ...
        ax5.text(x=ls["wl_ls"], y=9.6e-23, s=f"ls:{ls["wl_ls"]:.1f}", 
            ha="left", va="bottom", rotation=50, c=my_cm(i))
        ax5.text(x=ls["wl_on"], y=9.6e-23, s=f"on:{ls["wl_on"]:.1f}", 
            ha="left", va="bottom", rotation=50, c=my_cm(i))
        ax5.text(x=ls["wl_off"], y=9.6e-23, s=f"off:{ls["wl_off"]:.1f}", 
            ha="left", va="bottom", rotation=50, c=my_cm(i))
...
ax6.plot(spec, N(lidar_coord.z0)*10e-6*xs_SO2(spec), c="black")
ax6.plot(spec, N(lidar_coord.z0)*100e-6*xs_H2S(spec), c="black")

ax6.text(x=245, y=0.0162, va="center", ha="left", s=r"H$_2$S")
ax6.text(x=245, y=0.0145, va="center", ha="left", s=r"(100ppm)")

ax6.text(x=280, y=0.0127, va="center", ha="left", s=r"O$_3$")
ax6.text(x=280, y=0.011, va="center", ha="left", s=r"(1ppm)")

ax6.text(x=315, y=0.0047, va="center", ha="left", s=r"SO$_2$")
ax6.text(x=315, y=0.003, va="center", ha="left", s=r"(10ppm)")
...
ax4.set(
    xlabel= r"Laser wavelength [nm]",
    ylabel=r"Statistical errors |s$_{SO2}$| [ppm]",
)
ax5.set(
...
ext="png"
fig1.savefig(format=ext, dpi=400, bbox_inches="tight", fname="samples/sim_result/sim_01_power_6combies."+ext)
fig2.savefig(format=ext, dpi=400, bbox_inches="tight", fname="samples/sim_result/sim_01_n_SO2_6combies."+ext)
fig3.savefig(format=ext, dpi=400, bbox_inches="tight", fname="samples/sim_result/sim_01_contami_error_6combies."+ext)
fig4.savefig(format=ext, dpi=400, bbox_inches="tight", fname="samples/sim_result/sim_01_stat_error_6combies."+ext)
fig5.savefig(format=ext, dpi=400, bbox_inches="tight", fname="samples/sim_result/sim_01_cond_2case."+ext)
fig6.savefig(format=ext, dpi=400, bbox_inches="tight", fname="samples/sim_result/sim_01_3gases_absorptance."+ext)
fig7.savefig(format=ext, dpi=400, bbox_inches="tight", fname="samples/sim_result/sim_01_n_SO2_6combies_zoom."+ext)
