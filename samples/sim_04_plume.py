...
src = np.array([
    [1, 50,  -10,  lidar_coord.z0 + 2], 
    [1, 200, -50,  lidar_coord.z0 + 2], 
    [1, 400, -100, lidar_coord.z0 + 2], 
    [1, 600, -50,  lidar_coord.z0 + 2], 
    [1, 625, -100, lidar_coord.z0 + 2], 
    [1, 650, -50,  lidar_coord.z0 + 2], 
]).T
...
for (s1, s2), res_obj in results.items():
    ...
    ax_dist.errorbar(
        ...
        y=utils.number_density_to_ppm(res_obj.res[:, idx_trgt]-cfs[(s1, s2)][:, idx_trgt], res_obj.coord.z), 
        yerr=utils.number_density_to_ppm(res_obj.stat_err[:, idx_trgt], res_obj.coord.z),
        capsize=8, fmt='o', markersize=6, ecolor=my_cm(i), color=my_cm(i), label=f"{s1}-{s2}", zorder=num-i
    )
    ax_err1_wl.scatter(
        wl_laser, 
        ...
        color=my_cm(i), label=f"{s1}-{s2}", s=8, zorder=num-i
    )
    ax_err2_wl.scatter(
        wl_laser, 
        utils.number_density_to_ppm(res_obj.stat_err[-1, :], res_obj.coord.z[-1]), 
        color=my_cm(i), label=f"{s1}-{s2}", s=3, zorder=num-i
    )
...
ax_dist.legend(handles=color_handles, loc="upper right", frameon=True)
...
ax2.set_xlabel("(Line of Sight) distance [m]")
ax2.set_ylabel(r"received power $P_{phot}$")
