...
from gas_simulation.model import Field, Source, Gas, InstantEnvironment
from gas_simulation.lidar_model.lidar import (
    Coord,
    Lidar,
    Dial,
    calc_dial_correction_factor,
)
...
env = InstantEnvironment(
    ...
    time=t_sec,
    trig=lambda x: ((x >= 300) & (x <= 400)) | ((x >= 700) & (x <= 800)),
)
