...
# === power calculation ===
p_N2 = lidar.power(
    dist=lidar.coord.distance[:, np.newaxis],
...
axes[0].plot(lidar.coord.distance, tau_on[:, idx_300nm], c="red", ls="-", label="on")
axes[0].plot(
    lidar.coord.distance, tau_off[:, idx_300nm], c="blue", ls="-", label="off"
)
...
ax.scatter(
    dial.coord.distance,
    utils.number_density_to_ppm(res-dial_correction_factor, dial.coord.z[:, np.newaxis])[:, idx_300nm],
    c="green",
...
axes[0].plot(
    distance_sw, 
    p_on_sw[:, idx_300nm], 
    marker="*", c="red", ls="-", label="on (5pt sum)"
)
axes[0].plot(
    distance_sw,
    p_off_sw[:, idx_300nm],
    marker="*", c="blue", ls="-", label="off (5pt sum)",
)
...
dial = Dial(lidar, distance=(distance_sw[5:]+distance_sw[:-5])/2, dR=distance_sw[5:]-distance_sw[:-5])
