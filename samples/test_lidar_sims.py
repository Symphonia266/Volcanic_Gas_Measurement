...
p_N2 = env.lidar.power(
    dist=env.lidar.distance[1:, np.newaxis],
...
dist = np_SWV(env.lidar.distance[1:], window_shape=5).mean(axis=-1)
...
# === DIAL calculation ===
dial = Dial(env.lidar)
dR = env.lidar.distance[2:] - env.lidar.distance[1:-1]
res = dial.calc(p_on, p_off, dR[:, np.newaxis], d_xs_SO2[np.newaxis, :])
...
alpha_mol_on = alpha_mol_laser + alphas_mol(
    wl_on[np.newaxis, :], dial.z_grid[:, np.newaxis]
)
alpha_mol_off = alpha_mol_laser + alphas_mol(
    wl_off[np.newaxis, :], dial.z_grid[:, np.newaxis]
)
d_alpha_mol = alpha_mol_on - alpha_mol_off

alpha_aer_laser = alphas_aer(
    wl_laser[np.newaxis, :], dial.z_grid[:, np.newaxis], env.aer_absorp_feat
)
alpha_aer_on = alpha_aer_laser + alphas_aer(
    wl_on[np.newaxis, :], dial.z_grid[:, np.newaxis], env.aer_absorp_feat
)
alpha_aer_off = alpha_aer_laser + alphas_aer(
    wl_off[np.newaxis, :], dial.z_grid[:, np.newaxis], env.aer_absorp_feat
)
...
axes[0].plot(
    env.lidar.distance[1:], tau_off[:, idx_300nm], c="blue", ls="-", label="off"
)
...
axes[1].plot(
    env.lidar.distance[1:], p_on[:, idx_300nm], marker="o", c="red", ls="-", label="on"
)
axes[1].plot(
    env.lidar.distance[1:],
    p_off[:, idx_300nm],
    marker="o",
    c="blue",
    ls="-",
...
axes[1].plot(dist, p_on_sw[:, idx_300nm], marker="o", c="red", ls="-", label="on")
axes[1].plot(
    dist,
    p_off_sw[:, idx_300nm],
    marker="o",
...
ax.scatter(
    ...
    utils.number_density_to_ppm(
        env.number_density_at(dial.x_grid, 0, dial.z_grid)["SO2"], dial.z_grid
    ),
    c="black",
)
ax.scatter(
...
ax.scatter(
    dial.distance,
    utils.number_density_to_ppm(
        res - dial_correction_factor, dial.z_grid[:, np.newaxis]
    )[:, idx_300nm],
    c="green",
